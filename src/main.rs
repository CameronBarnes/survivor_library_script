...
use parsing::{get_page_from_path, parse_document_page, parse_main_page};
...
fn get_categories() -> Vec<LibraryItem> {
    parse_main_page(&get_page_from_path("/library-download.html"))
        .par_iter()
        .map(|(name, path)| {
...
fn main() {
    let category = LibraryItem::Category(Category::new(
        "Survivor Library".to_string(),
        get_categories(),
        false,
    ));
